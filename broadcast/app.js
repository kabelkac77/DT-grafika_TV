...
(() => {
  ...
  /* Srovná plochu s tím, co na ní má být. items = [{gid, payload}] odspodu nahoru. */
  function syncSurface(name, items) {
    ...
    items.forEach((item, index) => {
      ...
      if (frame.hidden) { frame.hidden = false; clearTimeout(frame.hider); }
  ...
  function hideLater(frame) {
    ...
    frame.hider = setTimeout(() => { frame.hidden = true; frame.sent = ''; }, 340);
  }
  ...
  function arm(gid, { keepQueue = false } = {}) {
    ...
    state.preview = {
      ...
      duration: g.duration, autoOut: g.duration > 0,
  ...
  function take() {
    ...
    for (const layer of [...collisions(p.gid), ...state.program.filter(l => l.gid === p.gid)]) {
      drop(layer, layer.gid === p.gid ? 'nahrazeno novými daty' : 'kolize zóny s ' + p.gid);
    }

    const layer = {
      gid: p.gid, payload: p.payload, targets: { ...p.targets },
      label: objectLabel(p.gid), edited: p.edited,
      deadline: p.autoOut && p.duration > 0 ? Date.now() + p.duration * 1000 : null
    };
    state.program.push(layer);
    if (layer.deadline) layer.timer = setTimeout(() => drop(layer, 'auto out'), p.duration * 1000);

    const targets = [p.targets.tv && 'TV', p.targets.led && 'LED'].filter(Boolean).join(' + ');
    log('take', p.gid, `TAKE → ${targets} · ${layer.label}` + (p.edited ? ' · ručně upraveno' : ''));
  ...
  function drawFrames() {
    syncSurface('pv', state.preview ? [{ gid: state.preview.gid, payload: state.preview.payload }] : []);
    syncSurface('pg', state.program.map(l => ({ gid: l.gid, payload: l.payload })));
    syncSurface('tv', state.program.filter(l => l.targets.tv).map(l => ({ gid: l.gid, payload: l.payload })));
    syncSurface('led', state.program.filter(l => l.targets.led).map(l => ({ gid: l.gid, payload: l.payload })));
  ...
  $('duration').oninput = () => { if (state.preview) { state.preview.duration = Number($('duration').value) || 0; draw(); } };
