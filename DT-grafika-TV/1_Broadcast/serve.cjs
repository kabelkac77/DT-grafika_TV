/* Drobný statický server pro místní zkoušku režijního pultu.
   ...
   Kořenem je celý repozitář: karty sahají na ../../design-system/,
   který leží nad složkou DT-grafika-TV/. */
...
http.createServer((request, response) => {
  ...
  if (file !== root && !file.startsWith(root + path.sep)) { response.writeHead(403).end('403'); return; }
...
}).listen(port, () => console.log(`SVDT — režijní pult: http://localhost:${port}/DT-grafika-TV/1_Broadcast/`));
